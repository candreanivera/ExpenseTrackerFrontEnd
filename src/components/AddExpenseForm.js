...
function AddExpenseForm({ onAdd }) {
  ...
  const handleSubmit = e => {
    ...
    fetch('http://localhost:5186/api/expenses', {
      method: 'POST',
  ...
  return (
    <div className='addExpense'>
      <div className={`error-message ${!errors || (typeof errors === 'object' && Object.keys(errors).length === 0) ? 'error-hidden' : ''}`}>
        {typeof errors === 'string'
          ? errors
          : Object.values(errors).filter(msg => msg).join(' | ')}
      </div>
      <h2 className = 'subtitulo' style={{paddingBottom: '10px'}} >🪙 Add Expense</h2>
      <form onSubmit={handleSubmit} className='form'>
        <div className='form-group'>
          <label className='addExpenseLabel'>Expense Detail:</label>
          <input name="description" placeholder="Expense Detail" value={form.description} onChange={handleChange} />
        </div>
        <div className='form-group'>
        ...
        <div className='form-group'>
          <label className='addExpenseLabel'>Date: </label>
            <DatePicker selected={form.date && !isNaN(new Date(form.date).getTime()) ? new Date(form.date) : null} 
                        onChange={(date) => {
                        if (date) {
